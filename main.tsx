...
import "./styles.css";
import { CITIES, BOARD_ORDER, PLAYER_COLORS, GROUP_COLORS, GROUP_LABELS, MAX_DEVELOPMENT_LEVEL } from "./cities";
    ...
    <div className="route-track card">
      <div className="route-track-inner">
        {BOARD_ORDER.map((cityId, index) => {
          ...
          const groupColor = GROUP_COLORS[city.group];
          const borderColor = owner ? owner.color : groupColor;
          return (
            <div
              key={cityId}
              className="route-pill"
              style={{ borderColor, background: owner ? `${owner.color}22` : `${groupColor}14` }}
              title={GROUP_LABELS[city.group]}
            >
  ...
  return (
    <div className="ledger-bar card">
        ...
        {game.players.map((p) => (
          <div key={p.id} className={`ledger-player${p.id === current?.id ? " is-current" : ""}${p.bankrupt ? " is-bankrupt" : ""}`}>
            ...
            <span className="ledger-name">
              {p.name}{p.isBot && " 🤖"}{!p.connected && !p.isBot && " (getrennt)"}{p.inJail && " 🔒"}
            </span>
      ...
      <div className="ledger-action">
        ...
        {isYourTurn && game.turnPhase === "awaiting_roll" && current?.inJail && (
          <>
            <span className="ledger-waiting">Im Gefängnis — Pasch würfeln zum Freikommen</span>
            <button className="btn" onClick={onPayBail}>Kaution zahlen (€200)</button>
          </>
        )}
...
/* ======================= PropertiesPanel (Häuser/Hotels bauen) ======================= */
...
function PropertiesPanel({
  ...
  function ownsGroup(group: string): boolean {
    return Object.values(CITIES).filter((c) => c.group === group).every((c) => game.cities[c.id]?.ownerId === youId);
  }

  return (
    <div className="deed-overlay" onClick={onClose}>
      <div className="deed card build-card" onClick={(e) => e.stopPropagation()}>
        <span className="deed-eyebrow">Deine Städte</span>
        <h2 className="display" style={{ marginTop: 0 }}>Häuser &amp; Hotels bauen</h2>
        <p className="lobby-hint">Baue nur, wenn du die komplette Farbgruppe einer Stadt besitzt.</p>
        {owned.length === 0 && <p className="lobby-hint">Du besitzt noch keine Städte.</p>}
        <div className="build-list">
          {owned.map((c) => {
            const cityDef = CITIES[c.id];
            const hasGroup = ownsGroup(cityDef.group);
            const cost = Math.round(cityDef.price * 0.5);
            const maxed = c.developmentLevel >= MAX_DEVELOPMENT_LEVEL;
            const label =
              c.developmentLevel === 0 ? "Unbebaut" : c.developmentLevel >= MAX_DEVELOPMENT_LEVEL ? "Hotel" : `${c.developmentLevel} Haus/Häuser`;
            return (
              <div key={c.id} className="build-row" style={{ borderColor: GROUP_COLORS[cityDef.group] }}>
                <div className="build-row-info">
                  <strong>{cityDef.name}</strong>
                  <span className="lobby-hint">{GROUP_LABELS[cityDef.group]} · {label}</span>
                </div>
                <button
                  className="btn btn-primary"
                  disabled={!hasGroup || maxed || !you || you.money < cost}
                  title={!hasGroup ? "Du brauchst die komplette Farbgruppe" : undefined}
                  onClick={() => onBuild(c.id)}
                >
                  {maxed ? "Maximal ausgebaut" : `Bauen (€${cost.toLocaleString("de-DE")})`}
                </button>
              </div>
            );
          })}
    ...
    <div className="game-shell">
      <header className="topbar card">
        ...
        <div className="topbar-actions">
          <button className="btn" onClick={() => setBuildOpen(true)}>🏗️ Bauen</button>
      ...
      {buildOpen && (
        <PropertiesPanel game={state} youId={youId} onBuild={(cityId) => send({ type: "build_house", cityId })} onClose={() => setBuildOpen(false)} />
      )}
