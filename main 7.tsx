...
import { ConquestApp } from "./conquest";
...
function Lobby({ onEnterRoom, onBack, game, youId, connected, onStartGame }: { onEnterRoom: (id: string, name: string, color: string) => void; onBack: () => void; game: GameState | null; youId: string | null; connected: boolean; onStartGame: () => void }) {
...
function EmpireApp({ onBack }: { onBack: () => void }) {
  ...
  if (!state || !youId || state.phase === "lobby") {
    return <Lobby onEnterRoom={handleEnterRoom} onBack={onBack} game={state} youId={youId} connected={connected} onStartGame={() => send({ type: "start_game" })} />;
  }
...
function ModeSelect({ onChoose }: { onChoose: (mode: "empire" | "conquest") => void }) {
  return (
    <div className="lobby-shell">
      <div className="lobby-card card mode-select-card">
        <span className="deed-eyebrow">Zwei Spiele, eine Welt</span>
        <h1 className="display">Was möchtest du spielen?</h1>
        <div className="mode-cards">
          <button className="mode-card" onClick={() => onChoose("empire")}>
            <span className="mode-card-title display">World Empire</span>
            <span className="mode-card-desc">Kaufe Städte, kassiere Miete, handle mit Mitspielern — Wirtschaftsspiel auf dem Globus.</span>
          </button>
          <button className="mode-card" onClick={() => onChoose("conquest")}>
            <span className="mode-card-title display">World Conquest</span>
            <span className="mode-card-desc">Erobere Territorien im Risiko-Stil — Würfel-Duelle auf einem echten 3D-Globus.</span>
          </button>
        </div>
      </div>
    </div>
...
function Root() {
  const [mode, setMode] = useState<"select" | "empire" | "conquest">("select");
  if (mode === "empire") return <EmpireApp onBack={() => setMode("select")} />;
  if (mode === "conquest") return <ConquestApp onBack={() => setMode("select")} />;
